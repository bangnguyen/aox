...
import scala.Some
import play.api.test.{WithApplication, FakeHeaders, FakeRequest, FakeApplication}
...
class UserRestServiceSpec extends Specification{
  ...
  "Create a new  users" should {
    ...
    "HTTP/1.1 403 Forbidden" in new WithApplication{
      ...
      status(result1) must equalTo(NOT_ACCEPTABLE)
